...
class TableActivity : AppCompatActivity() {
    ...
    private val cellDp = 90
    private val gapDp = 12

    private var editMode = false
    private val planCache = linkedMapOf<String, TablePlan>()

    // ✅ Render sicher machen (keine Layout/Measure Crashes)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // ✅ Plus-Button (oben rechts) -> Tisch hinzufügen
    ...
    private fun renderFromCacheInternal() {
        ...
        for ((tableId, raw) in planCache) {
            val plan = clampPlanToCanvas(raw)
    ...
    private fun createTableButton(tableId: String, plan: TablePlan): Button {
        ...
        if (!editMode) {
            // ✅ Normalmodus: Bestellseite öffnen
            ...
            // ✅ Editmodus: Tap -> Menü (Umbenennen/Größe/Löschen), LongPress -> Drag
    ...
    /**
     * ✅ Stabil:
     * - Single Tap -> Menü (Umbenennen / Größe / Löschen)
     * - Long Press -> Drag aktiv
     * - Move -> verschieben + Raster-Snap
     */
            ...
            when (event.actionMasked) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    if (dragging) {
                        ...
                        // ✅ magnetisch: aufs Raster snappen
                        val (gx, gy) = pxToGrid(newLeft, newTop)
                        lp.leftMargin = dp(gx * (cellDp + gapDp)).coerceIn(0, maxLeft)
                        lp.topMargin = dp(gy * (cellDp + gapDp)).coerceIn(0, maxTop)
                ...
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                    if (dragging) {
                        ...
                        planCache[tableId] = clampPlanToCanvas(cur.copy(x = gx, y = gy))
    ...
    private fun addNewTable() {
        ...
        val (fx, fy) = findFirstFreeSpot(w = 1, h = 1)
    ...
    private fun findFirstFreeSpot(w: Int, h: Int): Pair<Int, Int> {
        val maxX = maxGridXFor(w)
        val maxY = maxGridYFor(h)
    ...
    private fun showResizeDialog(tableId: String) {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Breite +") { _, _ ->
                planCache[tableId] = clampPlanToCanvas(current.copy(w = (current.w + 1).coerceAtMost(6)))
                scheduleRender()
            }
            .setNeutralButton("Breite -") { _, _ ->
                planCache[tableId] = clampPlanToCanvas(current.copy(w = (current.w - 1).coerceAtLeast(1)))
                scheduleRender()
            }
    ...
    private fun showResizeDialogHeight(tableId: String) {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Höhe +") { _, _ ->
                planCache[tableId] = clampPlanToCanvas(current.copy(h = (current.h + 1).coerceAtMost(6)))
                scheduleRender()
            }
            .setNeutralButton("Höhe -") { _, _ ->
                planCache[tableId] = clampPlanToCanvas(current.copy(h = (current.h - 1).coerceAtLeast(1)))
                scheduleRender()
            }
    ...
    private fun seedLayoutIfEmpty() {
        tablesRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                when (roomName.trim()) {
                    "Restaurant" -> {
                        add(updates, "7", 0, 0, 1, 1)
                        add(updates, "6", 1, 0, 1, 1)
                        add(updates, "5", 2, 0, 1, 1)
                        add(updates, "4", 3, 0, 1, 1)

                        add(updates, "3", 1, 1, 1, 1)
                        add(updates, "2", 2, 1, 1, 1)
                        add(updates, "1", 3, 1, 1, 1)

                        add(updates, "10", 0, 3, 1, 2)
                        add(updates, "9", 1, 3, 1, 1)
                        add(updates, "8", 3, 3, 1, 1)

                        add(updates, "11", 1, 6, 1, 1)
                        add(updates, "12", 2, 6, 1, 1)
                        add(updates, "13", 4, 6, 1, 2)
                    }

                    "Gewölbe" -> {
                        add(updates, "17", 1, 0, 1, 1)
                        add(updates, "18", 3, 0, 1, 1)
                        add(updates, "14", 0, 2, 1, 3)
                        add(updates, "15", 2, 2, 1, 3)
                        add(updates, "16", 4, 2, 1, 3)
                    }
                    ...
                    "Scheune EG" -> {
                        ...
                        add(updates, "35", 2, 4, 3, 1)
                        add(updates, "36", 6, 3, 3, 1)
                    }

                    "Scheune UG" -> {
                        add(updates, "40", 0, 3, 1, 1)
                        add(updates, "41", 1, 1, 1, 1)
                        add(updates, "42", 2, 2, 1, 1)
                        add(updates, "43", 3, 1, 1, 1)
                        add(updates, "44", 4, 0, 1, 2)
                    }

                    "Terrasse" -> {
                        add(updates, "60", 0, 0, 1, 1)
                        add(updates, "61", 1, 0, 1, 1)
                        add(updates, "62", 2, 0, 1, 1)

                        add(updates, "63", 0, 2, 2, 1)
                        add(updates, "64", 2, 2, 2, 1)
                        add(updates, "65", 3, 1, 1, 1)

                        add(updates, "66", 5, 2, 3, 1)
                        add(updates, "67", 5, 3, 3, 1)
                    }
    ...
    private fun frameParamsFor(plan: TablePlan): FrameLayout.LayoutParams {
        val width = dp(plan.w * cellDp + (plan.w - 1) * gapDp)
        val height = dp(plan.h * cellDp + (plan.h - 1) * gapDp)

        return FrameLayout.LayoutParams(width, height).apply {
            leftMargin = dp(plan.x * (cellDp + gapDp))
            topMargin = dp(plan.y * (cellDp + gapDp))
        }
    }

    private fun clampPlanToCanvas(plan: TablePlan): TablePlan {
        val newW = plan.w.coerceAtLeast(1)
        val newH = plan.h.coerceAtLeast(1)

        val maxX = maxGridXFor(newW)
        val maxY = maxGridYFor(newH)
    ...
    private fun maxGridXFor(w: Int): Int {
        val canvasW = getCanvasWidthPx()
        val tableWpx = dp(w * cellDp + (w - 1) * gapDp)
        val maxLeftPx = (canvasW - tableWpx).coerceAtLeast(0)
        val stepPx = dp(cellDp + gapDp).coerceAtLeast(1)
        return (maxLeftPx.toFloat() / stepPx).toInt().coerceAtLeast(0)
    }

    private fun maxGridYFor(h: Int): Int {
        val canvasH = getCanvasHeightPx()
        val tableHpx = dp(h * cellDp + (h - 1) * gapDp)
        val maxTopPx = (canvasH - tableHpx).coerceAtLeast(0)
        val stepPx = dp(cellDp + gapDp).coerceAtLeast(1)
        return (maxTopPx.toFloat() / stepPx).toInt().coerceAtLeast(0)
    }
    ...
    private fun pxToGrid(leftPx: Int, topPx: Int): Pair<Int, Int> {
        val step = dp(cellDp + gapDp).coerceAtLeast(1)
        val x = (leftPx.toFloat() / step).toInt().coerceAtLeast(0)
        val y = (topPx.toFloat() / step).toInt().coerceAtLeast(0)
        return x to y
    }
